...
#include "assimp/private/AssetLib/Assjson/mesh_splitter.h"

extern "C" {
#include "assimp/private/AssetLib/Assjson/cencode.h"
}
namespace Assimp {
...
private:
    ...
    std::stringstream &LiteralToString(std::stringstream &stream, float f) {
        ...
        if (std::numeric_limits<float>::infinity() == std::fabs(f)) {
